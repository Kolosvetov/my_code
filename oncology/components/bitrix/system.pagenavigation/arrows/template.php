...
if($arResult["bDescPageNumbering"] === true)
{
	...
	?>
			<span class="b-pager__page current"><?=$NavRecordGroupPrint?></span>
	<?
			elseif($arResult["nStartPage"] == $arResult["NavPageCount"] && $arResult["bSavePage"] == false):
	?>
			<a class="b-pager__page" href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><?=$NavRecordGroupPrint?></a>
	<?
			else:
	?>
			<a class="b-pager__page" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=$arResult["nStartPage"]?>"><?=$NavRecordGroupPrint?></a>
	<?
			endif;
			$bFirst = false;
			$bPoints = true;
...
else
{
	...
	?>
		<!--<div class="navigation-arrows">-->
			<?if ($bPrevDisabled):?><span class="b-pager__prev"></span>
			<?else:?>
			<a href="<?=$sPrevHref;?>" class="b-pager__prev" id="<?=$ClientID?>_previous_page"></a>
			<?endif;?>
			<?if ($bNextDisabled):?><span  class="b-pager__next"></span>
			<?else:?><a href="<?=$sNextHref;?>" class="b-pager__next" id="<?=$ClientID?>_next_page"></a>
			<?endif;?>
		<!--</div>-->
	...
	do
	{
		if ($arResult["nStartPage"] <= 2 || $arResult["nEndPage"]-$arResult["nStartPage"] <= 1 || abs($arResult['nStartPage']-$arResult["NavPageNomer"])<=2)
		{
			...
			if ($arResult["nStartPage"] == $arResult["NavPageNomer"]):
	?><span class="b-pager__page current"><?=$arResult["nStartPage"]?></span>
	<?
			elseif($arResult["nStartPage"] == 1 && $arResult["bSavePage"] == false):
	?>
<a class="b-pager__page" href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><?=$arResult["nStartPage"]?></a>
	<?
			else:
	?>
			<a  class="b-pager__page"href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=$arResult["nStartPage"]?>"><?=$arResult["nStartPage"]?></a>
	<?
			endif;
			$bFirst = false;
			$bPoints = true;
